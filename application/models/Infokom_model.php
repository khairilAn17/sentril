...
class Infokom_model extends CI_Model{
	...
	function get_all_data_2018(){
		$query = $this->db->query("SELECT * FROM tbl_kegiatan_infokom WHERE tanggal LIKE '_2018'");
	}
	...
	function cari_kegiatan($id){
		return $this->db->select('*')->from('tbl_kegiatan_infokom')
			->join('tbl_subkegiatan_infokom','tbl_kegiatan_infokom.id_kegiatan=tbl_subkegiatan_infokom.id_kegiatan')
			->where('tbl_kegiatan_infokom.id_kegiatan',$id)->where('status','terverifikasi')->get();
	}

	function get_subkegiatan($id){
		return $this->db->where(array('id_kegiatan'=>$id,'status'=>'terverifikasi'))->get('tbl_subkegiatan_infokom');
	}


	function get_kegiatan($id){
		return $this->db->where('id_kegiatan',$id)->get('tbl_kegiatan_infokom');
	}


	function update_kegiatan($data,$id){
		return $this->db->where('id_kegiatan',$id)->update('tbl_kegiatan_infokom',$data);
	}

	function get_file_sub($id){
		return $this->db->select('file')->where('id_subkegiatan',$id)->get('tbl_subkegiatan_infokom');
	}
	function get_file_keg($id){
		return $this->db->select('file')->where('id_kegiatan',$id)->get('tbl_subkegiatan_infokom');
	}
	...
	function get_file_date($limit, $offset){
		return $this->db->select('tanggal_input')->from('tbl_subkegiatan_infokom')->group_by('tanggal_input')->order_by('tanggal_input','desc')->where('status','terverifikasi')->limit($limit,$offset)->get();
	}

	function get_file_total(){
		return $this->db->select('tanggal_kegiatan')->from('tbl_subkegiatan_infokom')->group_by('tanggal_kegiatan')->order_by('tanggal_kegiatan','desc')->where('status','terverifikasi')->get();
	}
	function get_file($date){
		return $this->db->where('tanggal_input',$date)->get('tbl_subkegiatan_infokom');
	}


	function get_kegiatan_su(){
		return $this->db->query("SELECT *,tbl_kegiatan_infokom.anggaran as anggaran2,count(tbl_subkegiatan_infokom.anggaran) as realisasi2, sum(tbl_subkegiatan_infokom.anggaran) as jlh_anggaran FROM `tbl_kegiatan_infokom` join tbl_subkegiatan_infokom on tbl_kegiatan_infokom.id_kegiatan=tbl_subkegiatan_infokom.id_kegiatan where status='terverifikasi' group by tbl_kegiatan_infokom.id_kegiatan");
	}
	...
	function get_total_kegiatan2($id){
		return $this->db->query("select count(id_kegiatan) as total_kegiatan, sum(anggaran) as total_anggaran from tbl_kegiatan_infokom where id_kegiatan='$id'");
	}
	function get_datetime(){
		$query = $this->db->query("SELECT tanggal FROM tbl_kegiatan_infokom");
		return $query->result();
	}
	function get_group($tahun){
		$query =  $this->db->query("SELECT * FROM tbl_kegiatan_infokom WHERE tanggal LIKE '%$tahun%' GROUP BY(nama_pj)");
		return $query->result();
	}
}