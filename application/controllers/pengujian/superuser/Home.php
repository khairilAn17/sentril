...
class Home extends CI_Controller {
	...
	{
		$this->load->view('pengujian/spuser/templates/header_home');
		$this->load->view('pengujian/spuser/home');
		$this->load->view('pengujian/spuser/templates/footer_home');
	}
	...
	public function kegiatan($param="")
	{
		// if($param==2018){
			...
			if(isset($_POST['ubah'])){
				...
				if($this->pengujian_model->update_kegiatan($data,$id)){
					$data['pesan'] = 1;
				}else{
					$data['pesan'] = 0;
				}
				...
				//$data['row'] = $this->pengujian_model->get_kegiatan_su()->result_array();
		$data['row'] = $this->pengujian_model->get_all_data("tbl_kegiatan_pengujian","tanggal", $param)->result_array();
		//$data['row'] = $this->pengujian_model->get_all_data_2018();
		$data['total'] = $this->pengujian_model->get_total_kegiatan($param)->row_array();
		$data['subtotal'] = $this->pengujian_model->get_total_subkegiatan($param)->row_array();
		$data['group'] = $this->pengujian_model->get_group($param);
		$data['tahun'] = $this->db->query('SELECT * FROM tbl_tahun')->result();
		$this->session->set_flashdata('tahun', $param);

		
		//var_dump($data);die;
		$this->load->view('pengujian/spuser/templates/header_table');
		$this->load->view('pengujian/spuser/kegiatan',$data);
		$this->load->view('pengujian/spuser/templates/footer_table');
		$this->load->view('pengujian/spuser/kegiatan_script');
		// }
	...
	public function user()
	{
		if(isset($_POST['ubah'])){
			...
			if($this->pengujian_model->update_user($data,$id)){
				$data['pesan'] = 1;
			}else{
				$data['pesan'] = 0;
			}
		...
		$data['row'] = $this->pengujian_model->get_all_data_user("tbl_user","pengujian")->result_array();
		//var_dump($data);die;
		$this->load->view('pengujian/spuser/templates/header_table');
		$this->load->view('pengujian/spuser/user',$data);
		$this->load->view('pengujian/spuser/templates/footer_table');
		$this->load->view('pengujian/spuser/user_script');
	}

	public function add_user()
	{
		$this->load->view('pengujian/spuser/templates/header_table');
		$this->load->view('pengujian/spuser/add_user');
		$this->load->view('pengujian/spuser/templates/footer_table');
	}
	...
	public function insert_proccess_user(){
		...
		$this->pengujian_model->insert_data("tbl_user",$data);
	...
	public function tambah_anggaran($tahun="")
	{
		...
		$data['tahun'] = $this->db->query('SELECT * FROM tbl_tahun')->result();
		$this->load->view('pengujian/spuser/templates/header_insert');
		$this->load->view('pengujian/spuser/add_anggaran',$data);
		$this->load->view('pengujian/spuser/templates/footer_insert');
	}
	...
	{	
		...
		$data['tahun'] = $this->db->query('SELECT * FROM tbl_tahun')->result();
		$this->load->view('pengujian/spuser/templates/header_insert');
		$this->load->view('pengujian/spuser/min_anggaran',$data);
		$this->load->view('pengujian/spuser/templates/footer_insert');
	}
	...
	public function kurangi_anggaran_proccess(){
		...
		$trget = str_replace(".", "",$target);
		$anggrn = str_replace(".", "",$anggaran);
		$this->pengujian_model->db->query("UPDATE tbl_kegiatan_pengujian SET target=target-$trget,anggaran=anggaran-$anggrn,sisa_anggaran=sisa_anggaran-$anggrn,sisa_target=sisa_target-$trget WHERE id_kegiatan='$id'");
		$this->pengujian_model->db->query("UPDATE tbl_anggaran_all SET target=target-$trget,anggaran=anggaran-$anggrn,sisa_anggaran=sisa_anggaran-$anggrn,sisa_target=sisa_target-$trget WHERE id_kegiatan='$id'");
		redirect('pengujian/superuser/home/kegiatan/2019');
	}
	...
	{
		$data['row'] = $this->pengujian_model->get_all_data("tbl_subkegiatan_pengujian","tahun", 2019)->result_array();
		//var_dump($data);die;
		$this->load->view('pengujian/spuser/templates/header_table');
		$this->load->view('pengujian/spuser/subkegiatan',$data);
		$this->load->view('pengujian/spuser/templates/footer_table');
	}

	public function add_kegiatan(){
		$this->load->view('pengujian/spuser/templates/header_insert');
		$this->load->view('pengujian/spuser/add_kegiatan');
		$this->load->view('pengujian/spuser/templates/footer_insert');
	}
	...
	public function insert_proccess(){
		...
		$this->pengujian_model->insert_data("tbl_kegiatan_pengujian",$data);
		$this->pengujian_model->insert_data("tbl_anggaran_all",$data1);
		redirect('pengujian/superuser/home/kegiatan/2019');
	}
	public function delete(){
		$id = $this->input->post('checkbox');# Using Form POST method you can use whatever you want like GET
		for ($i=0; $i < count($id) ; $i++) { 
			$gbr = $this->pengujian_model->get_file_keg($id[$i])->result_array();
			foreach($gbr as $data){
				$file = './assets/file/'.$data['file'];
				unlink($file);
			}
			$this->pengujian_model->delete_data("tbl_kegiatan_pengujian","id_kegiatan",$id[$i]);
			$this->pengujian_model->delete_data("tbl_subkegiatan_pengujian","id_kegiatan",$id[$i]);
			$this->pengujian_model->delete_data("tbl_anggaran_all","id_kegiatan",$id[$i]);
			$this->pengujian_model->delete_data("tbl_subangaran_all","id_kegiatan",$id[$i]);
		}
		redirect('pengujian/superuser/home/kegiatan/2019');
	}
	public function delete_subkegiatan(){
		...
		//var_dump($id);die;
		for ($i=0; $i < count($id) ; $i++) { 
			$gbr = $this->pengujian_model->get_file_sub($id[$i])->result_array();
			//var_dump($gbr);die;
			foreach($gbr as $data){
				$file = './assets/file/'.$data['file'];
				unlink($file);
			}
			$data_blk = $gbr = $this->pengujian_model->get_data("tbl_subkegiatan_pengujian","id_subkegiatan",$id[$i])->row();
			//var_dump($data_blk->status);die;
			if($data_blk->status=='terverifikasi'){
				$this->pengujian_model->db->query("UPDATE tbl_kegiatan_pengujian SET realisasi=realisasi-1,sisa_target=sisa_target+1,realisasi_anggaran=realisasi_anggaran-$data_blk->anggaran,sisa_anggaran=sisa_anggaran+$data_blk->anggaran WHERE id_kegiatan='$data_blk->id_kegiatan'");
				$this->pengujian_model->db->query("UPDATE tbl_anggaran_all SET realisasi=realisasi-1,sisa_target=sisa_target+1,realisasi_anggaran=realisasi_anggaran-$data_blk->anggaran,sisa_anggaran=sisa_anggaran+$data_blk->anggaran WHERE id_kegiatan='$data_blk->id_kegiatan'");
			}
			
			$this->pengujian_model->delete_data("tbl_subkegiatan_pengujian","id_subkegiatan",$id[$i]);
			$this->pengujian_model->delete_data("tbl_subangaran_all","id_subkegiatan",$id[$i]);
			
		}
		// $gbr = $this->pengujian_model->get_file_sub($id)->row_array();
		// $file = './assets/file/'.$gbr['file'];
		// unlink($file);

		// $stat = $this->pengujian_model->db->query("SELECT status FROM tbl_subkegiatan_pengujian WHERE id_subkegiatan='$id'")->row_array();
		// //var_dump($gbr);die;
		// if($stat['status']=='terverifikasi'){
		// $this->pengujian_model->db->query("UPDATE tbl_kegiatan_pengujian SET realisasi=realisasi-1,sisa_target=sisa_target+1,realisasi_anggaran=realisasi_anggaran-$agr,sisa_anggaran=sisa_anggaran+$agr WHERE id_kegiatan='$id_keg'");
		// $this->pengujian_model->delete_data("tbl_subkegiatan_pengujian","id_subkegiatan",$id);
		// }else{
		// 	$this->pengujian_model->delete_data("tbl_subkegiatan_pengujian","id_subkegiatan",$id);
		// }
		redirect('pengujian/superuser/home/kegiatan/2019');
	...
	public function delete_user(){
		...
		//var_dump($id);die;
		for ($i=0; $i < count($id) ; $i++) { 
			$this->pengujian_model->delete_data("tbl_user","id_user",$id[$i]);
		}
		redirect('pengujian/superuser/home/user');
	}


	function kegiatan_ajax($id){
		$data = $this->pengujian_model->get_kegiatan($id)->row_array();
	...
	function user_ajax($id){
		$data = $this->pengujian_model->get_user($id)->row_array();		
		header("Content-Type:application/json");
		echo json_encode($data);
	}
	...
	function laporan(){
		...
		$total_rows = $this->pengujian_model->get_file_total()->num_rows();
		$data = [];
		if($total_rows>0){
			$limit = 10;
			$page_num = $this->uri->segment(4);
			($page_num>1) ? $start = ($page_num-1)*$limit : $start = 0;	
			$data['file'] = $this->pengujian_model->get_file_date($limit, $start)->result_array();
		...
		$this->load->view('pengujian/spuser/templates/header_table');
		$this->load->view('pengujian/spuser/laporan',$data);
		$this->load->view('pengujian/spuser/templates/footer_table');
		$this->load->view('pengujian/spuser/laporan_script');
			
	}


	function file_laporan($date){
		$data['file'] = $this->pengujian_model->get_file($date)->result_array();
		$data['tanggal'] = $date;
		$this->load->view('pengujian/spuser/file_laporan',$data);
	}
	...
	public function verifikasi(){
		...
		//$agr = $this->pengujian_model->get_data("tbl_subkegiatan_pengujian","id_kegiatan",$id_keg)->row_array();
	    $this->pengujian_model->db->query("UPDATE tbl_subkegiatan_pengujian SET status='terverifikasi' WHERE id_subkegiatan='$id'");
	    $this->pengujian_model->db->query("UPDATE tbl_subangaran_all SET status='terverifikasi' WHERE id_subkegiatan='$id'");
		//$agr = $this->pengujian_model->db->query("SELECT anggaran FROM tbl_subkegiatan_pengujian WHERE id_kegiatan='$id_keg' AND status='terverifikasi' ORDER BY id_subkegiatan DESC LIMIT 1")->row_array();
		$agr = $this->pengujian_model->db->query("SELECT anggaran FROM tbl_subkegiatan_pengujian WHERE id_kegiatan='$id_keg' AND status='terverifikasi' AND id_subkegiatan='$id'")->row_array();
		$angg = $agr['anggaran'];
		//echo $angg;die;
		//var_dump($agr);die;
		/*$a = $this->pengujian_model->db->query("SELECT sum(anggaran) AS anggaran_total,count(id_kegiatan) AS jlh FROM tbl_subkegiatan_pengujian WHERE status='terverifikasi' AND  id_kegiatan='$id_keg'")->result_array();
		foreach ($a as $t) {
			$data[] = $t['jlh'];
		}
		//var_dump($data);die;
		...
		$this->pengujian_model->db->query("UPDATE tbl_kegiatan_pengujian SET realisasi=realisasi+1,sisa_target=sisa_target-1,realisasi_anggaran=realisasi_anggaran+$angg,sisa_anggaran=sisa_anggaran-$angg WHERE id_kegiatan='$id_keg'");
		
		$this->pengujian_model->db->query("UPDATE tbl_anggaran_all SET realisasi=realisasi+1,sisa_target=sisa_target-1,realisasi_anggaran=realisasi_anggaran+$angg,sisa_anggaran=sisa_anggaran-$angg WHERE id_kegiatan='$id_keg'");
		redirect('pengujian/superuser/home/log_subkegiatan');
	}	
	...
	public function cari_kegiatan(){
		...
		//var_dump($data);die;
		$this->load->view('pengujian/spuser/templates/header_table');
		$this->load->view('pengujian/spuser/cari_kegiatan');
		$this->load->view('pengujian/spuser/templates/footer_insert');
		$this->load->view('pengujian/spuser/cari_kegiatan_script');
	}
	...
	function cari_kegiatan_ajax($id){
		...
		$this->load->view('pengujian/spuser/cari_kegiatan_ajax',$data);
	}
	...
	function print_laporan2018($param=''){
		...
		$pj = $this->pengujian_model->get_all_data("tbl_kegiatan_pengujian", "tanggal", 2018)->result();
		foreach ($pj as $key) {	
				if($param== $key->nama_pj){
			       ...
			       $styleRight = array(
			        ...
			        $detail = $this->pengujian_model->get_all_data2019("tbl_kegiatan_pengujian", "nama_pj","tanggal", $param, 2018)->result_array();
					$total = $this->pengujian_model->get_total_kegiatan(2018)->row_array();
					$subtotal = $this->pengujian_model->get_total_subkegiatan(2018)->row_array();
	...
	function cetak_pdf2018($param=''){
		$pj = $this->pengujian_model->get_all_data("tbl_kegiatan_pengujian", "tanggal", 2018)->result();
		if ($param == '') {
				$data['row'] = $this->pengujian_model->get_all_data("tbl_kegiatan_pengujian","tanggal",2018)->result_array();
				$data['total'] = $this->pengujian_model->get_total_kegiatan(2018)->row_array();
				$data['subtotal'] = $this->pengujian_model->get_total_subkegiatan(2018)->row_array();
		...
		foreach ($pj as $key) {
			if ($param == $key->nama_pj) {
			$data['row'] = $this->pengujian_model->get_all_data2019("tbl_kegiatan_pengujian", "nama_pj", "tanggal", $param, 2018)->result_array();
			$data['total'] = $this->pengujian_model->get_total_kegiatan(2018)->row_array();
			$data['subtotal'] = $this->pengujian_model->get_total_subkegiatan(2018)->row_array();
			$this->load->view("pengujian/spuser/cetak",$data);
