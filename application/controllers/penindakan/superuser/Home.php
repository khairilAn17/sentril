...
class Home extends CI_Controller {
	...
	{
		$this->load->view('penindakan/spuser/templates/header_home');
		$this->load->view('penindakan/spuser/home');
		$this->load->view('penindakan/spuser/templates/footer_home');
	}
	...
	public function kegiatan($param="")
	{
		...
		$data['row'] = $this->penindakan_model->get_all_data("tbl_kegiatan_penindakan","tanggal", $param)->result_array();
		//$data['row'] = $this->penindakan_model->get_all_data_2018();
		$data['total'] = $this->penindakan_model->get_total_kegiatan($param)->row_array();
		$data['subtotal'] = $this->penindakan_model->get_total_subkegiatan($param)->row_array();
		$data['group'] = $this->penindakan_model->get_group($param);
		$data['tahun'] = $this->db->query('SELECT * FROM tbl_tahun')->result();
		$this->session->set_flashdata('tahun', $param);

		
		//var_dump($data);die;
		$this->load->view('penindakan/spuser/templates/header_table');
		$this->load->view('penindakan/spuser/kegiatan',$data);
		$this->load->view('penindakan/spuser/templates/footer_table');
		$this->load->view('penindakan/spuser/kegiatan_script');
	...
	public function user()
	{
		if(isset($_POST['ubah'])){
			...
			if($this->penindakan_model->update_user($data,$id)){
				$data['pesan'] = 1;
			}else{
				$data['pesan'] = 0;
			}
		...
		$data['row'] = $this->penindakan_model->get_all_data_user("tbl_user", "penindakan")->result_array();
		//var_dump($data);die;
		$this->load->view('penindakan/spuser/templates/header_table');
		$this->load->view('penindakan/spuser/user',$data);
		$this->load->view('penindakan/spuser/templates/footer_table');
		$this->load->view('penindakan/spuser/user_script');
	}

	public function add_user()
	{
		$this->load->view('penindakan/spuser/templates/header_table');
		$this->load->view('penindakan/spuser/add_user');
		$this->load->view('penindakan/spuser/templates/footer_table');
	}
	...
	public function insert_proccess_user(){
		...
		$this->penindakan_model->insert_data("tbl_user",$data);
	...
	{	
		...
		$data['tahun'] = $this->db->query('SELECT * FROM tbl_tahun')->result();
		$this->load->view('penindakan/spuser/templates/header_insert');
		$this->load->view('penindakan/spuser/add_anggaran',$data);
		$this->load->view('penindakan/spuser/templates/footer_insert');
	}
	...
	{	
		...
		$data['tahun'] = $this->db->query('SELECT * FROM tbl_tahun')->result();
		$this->load->view('penindakan/spuser/templates/header_insert');
		$this->load->view('penindakan/spuser/min_anggaran',$data);
		$this->load->view('penindakan/spuser/templates/footer_insert');
	}
	...
	public function kurangi_anggaran_proccess(){
		...
		$trget = str_replace(".", "",$target);
		$anggrn = str_replace(".", "",$anggaran);
		$this->penindakan_model->db->query("UPDATE tbl_kegiatan_penindakan SET target=target-$trget,anggaran=anggaran-$anggrn,sisa_anggaran=sisa_anggaran-$anggrn,sisa_target=sisa_target-$trget WHERE id_kegiatan='$id'");
		$this->penindakan_model->db->query("UPDATE tbl_anggaran_all SET target=target-$trget,anggaran=anggaran-$anggrn,sisa_anggaran=sisa_anggaran-$anggrn,sisa_target=sisa_target-$trget WHERE id_kegiatan='$id'");
		redirect('penindakan/superuser/home/kegiatan/2019');
	}
	...
	{
		$data['row'] = $this->penindakan_model->get_all_data("tbl_subkegiatan_penindakan","tahun", 2019)->result_array();
		//var_dump($data);die;
		$this->load->view('penindakan/spuser/templates/header_table');
		$this->load->view('penindakan/spuser/subkegiatan',$data);
		$this->load->view('penindakan/spuser/templates/footer_table');
	}

	public function add_kegiatan(){
		$this->load->view('penindakan/spuser/templates/header_insert');
		$this->load->view('penindakan/spuser/add_kegiatan');
		$this->load->view('penindakan/spuser/templates/footer_insert');
	}
	...
	public function insert_proccess(){
		...
		$this->penindakan_model->insert_data("tbl_kegiatan_penindakan",$data);
		$this->penindakan_model->insert_data("tbl_anggaran_all",$data1);
		redirect('penindakan/superuser/home/kegiatan/2019');
	}
	public function delete(){
		$id = $this->input->post('checkbox');# Using Form POST method you can use whatever you want like GET
		for ($i=0; $i < count($id) ; $i++) { 
			$gbr = $this->penindakan_model->get_file_keg($id[$i])->result_array();
			foreach($gbr as $data){
				$file = './assets/file/'.$data['file'];
				unlink($file);
			}
			$this->penindakan_model->delete_data("tbl_kegiatan_penindakan","id_kegiatan",$id[$i]);
			$this->penindakan_model->delete_data("tbl_subkegiatan_penindakan","id_kegiatan",$id[$i]);
			$this->penindakan_model->delete_data("tbl_anggaran_all","id_kegiatan",$id[$i]);
			$this->penindakan_model->delete_data("tbl_subangaran_all","id_kegiatan",$id[$i]);
		}
		redirect('penindakan/superuser/home/kegiatan/2019');
	}
	public function delete_subkegiatan(){
		...
		//var_dump($id);die;
		for ($i=0; $i < count($id) ; $i++) { 
			$gbr = $this->penindakan_model->get_file_sub($id[$i])->result_array();
			//var_dump($gbr);die;
			foreach($gbr as $data){
				$file = './assets/file/'.$data['file'];
				unlink($file);
			}
			$data_blk = $gbr = $this->penindakan_model->get_data("tbl_subkegiatan_penindakan","id_subkegiatan",$id[$i])->row();
			//var_dump($data_blk->status);die;
			if($data_blk->status=='terverifikasi'){
				$this->penindakan_model->db->query("UPDATE tbl_kegiatan_penindakan SET realisasi=realisasi-1,sisa_target=sisa_target+1,realisasi_anggaran=realisasi_anggaran-$data_blk->anggaran,sisa_anggaran=sisa_anggaran+$data_blk->anggaran WHERE id_kegiatan='$data_blk->id_kegiatan'");
				$this->penindakan_model->db->query("UPDATE tbl_anggaran_all SET realisasi=realisasi-1,sisa_target=sisa_target+1,realisasi_anggaran=realisasi_anggaran-$data_blk->anggaran,sisa_anggaran=sisa_anggaran+$data_blk->anggaran WHERE id_kegiatan='$data_blk->id_kegiatan'");
			}
			
			$this->penindakan_model->delete_data("tbl_subkegiatan_penindakan","id_subkegiatan",$id[$i]);
			$this->penindakan_model->delete_data("tbl_subangaran_all","id_subkegiatan",$id[$i]);
			
		}
		// $gbr = $this->penindakan_model->get_file_sub($id)->row_array();
		// $file = './assets/file/'.$gbr['file'];
		// unlink($file);

		// $stat = $this->penindakan_model->db->query("SELECT status FROM tbl_subkegiatan_penindakan WHERE id_subkegiatan='$id'")->row_array();
		// //var_dump($gbr);die;
		// if($stat['status']=='terverifikasi'){
		// $this->penindakan_model->db->query("UPDATE tbl_kegiatan_penindakan SET realisasi=realisasi-1,sisa_target=sisa_target+1,realisasi_anggaran=realisasi_anggaran-$agr,sisa_anggaran=sisa_anggaran+$agr WHERE id_kegiatan='$id_keg'");
		// $this->penindakan_model->delete_data("tbl_subkegiatan_penindakan","id_subkegiatan",$id);
		// }else{
		// 	$this->penindakan_model->delete_data("tbl_subkegiatan_penindakan","id_subkegiatan",$id);
		// }
		redirect('penindakan/superuser/home/kegiatan/2019');
	...
	public function delete_user(){
		...
		//var_dump($id);die;
		for ($i=0; $i < count($id) ; $i++) { 
			$this->penindakan_model->delete_data("tbl_user","id_user",$id[$i]);
		}
		redirect('penindakan/superuser/home/user');
	}


	function kegiatan_ajax($id){
		$data = $this->penindakan_model->get_kegiatan($id)->row_array();
	...
	function user_ajax($id){
		$data = $this->penindakan_model->get_user($id)->row_array();		
		header("Content-Type:application/json");
		echo json_encode($data);
	}
	...
	function laporan(){
		...
		$total_rows = $this->penindakan_model->get_file_total()->num_rows();
		$data = [];
		if($total_rows>0){
			$limit = 10;
			$page_num = $this->uri->segment(4);
			($page_num>1) ? $start = ($page_num-1)*$limit : $start = 0;	
			$data['file'] = $this->penindakan_model->get_file_date($limit, $start)->result_array();
			//var_dump($data['file']);die;
			$config['base_url'] = base_url('penindakan/superuser/home/laporan');
		...
		$this->load->view('penindakan/spuser/templates/header_table');
		$this->load->view('penindakan/spuser/laporan',$data);
		$this->load->view('penindakan/spuser/templates/footer_table');
		$this->load->view('penindakan/spuser/laporan_script');
			
	}


	function file_laporan($date){
		$data['file'] = $this->penindakan_model->get_file($date)->result_array();
		$data['tanggal'] = $date;
		$this->load->view('penindakan/spuser/file_laporan',$data);
	}
	...
	public function verifikasi(){
		...
		//$agr = $this->penindakan_model->get_data("tbl_subkegiatan_penindakan","id_kegiatan",$id_keg)->row_array();
	    $this->penindakan_model->db->query("UPDATE tbl_subkegiatan_penindakan SET status='terverifikasi' WHERE id_subkegiatan='$id'");
	    $this->penindakan_model->db->query("UPDATE tbl_subangaran_all SET status='terverifikasi' WHERE id_subkegiatan='$id'");
		//$agr = $this->penindakan_model->db->query("SELECT anggaran FROM tbl_subkegiatan_penindakan WHERE id_kegiatan='$id_keg' AND status='terverifikasi' ORDER BY id_subkegiatan DESC LIMIT 1")->row_array();
		$agr = $this->penindakan_model->db->query("SELECT anggaran FROM tbl_subkegiatan_penindakan WHERE id_kegiatan='$id_keg' AND status='terverifikasi' AND id_subkegiatan='$id'")->row_array();
		$angg = $agr['anggaran'];
		//echo $angg;die;
		//var_dump($agr);die;
		/*$a = $this->penindakan_model->db->query("SELECT sum(anggaran) AS anggaran_total,count(id_kegiatan) AS jlh FROM tbl_subkegiatan_penindakan WHERE status='terverifikasi' AND  id_kegiatan='$id_keg'")->result_array();
		foreach ($a as $t) {
			$data[] = $t['jlh'];
		}
		//var_dump($data);die;
		...
		$this->penindakan_model->db->query("UPDATE tbl_kegiatan_penindakan SET realisasi=realisasi+1,sisa_target=sisa_target-1,realisasi_anggaran=realisasi_anggaran+$angg,sisa_anggaran=sisa_anggaran-$angg WHERE id_kegiatan='$id_keg'");
		
		$this->penindakan_model->db->query("UPDATE tbl_anggaran_all SET realisasi=realisasi+1,sisa_target=sisa_target-1,realisasi_anggaran=realisasi_anggaran+$angg,sisa_anggaran=sisa_anggaran-$angg WHERE id_kegiatan='$id_keg'");
		redirect('penindakan/superuser/home/log_subkegiatan');
	}	

	public function cari_kegiatan(){
		//$data['row'] = $this->penindakan_model->get_all_data("tbl_kegiatan_penindakan")->result_array();
		//var_dump($data);die;
		$this->load->view('penindakan/spuser/templates/header_table');
		$this->load->view('penindakan/spuser/cari_kegiatan');
		$this->load->view('penindakan/spuser/templates/footer_insert');
		$this->load->view('penindakan/spuser/cari_kegiatan_script');
	}
	...
	function cari_kegiatan_ajax($id){
		...
		$this->load->view('penindakan/spuser/cari_kegiatan_ajax',$data);
	}
	...
	function print_laporan2018($param=''){
		...
		$pj = $this->penindakan_model->get_all_data("tbl_kegiatan_penindakan", "tanggal", 2018)->result();
		foreach ($pj as $key) {	
				if($param== $key->nama_pj){
			       ...
			       $styleRight = array(
			        ...
			        $detail = $this->penindakan_model->get_all_data2019("tbl_kegiatan_penindakan", "nama_pj","tanggal", $param, 2018)->result_array();
					$total = $this->penindakan_model->get_total_kegiatan(2018)->row_array();
					$subtotal = $this->penindakan_model->get_total_subkegiatan(2018)->row_array();
	...
	function cetak_pdf2018($param=''){
		...
		foreach ($pj as $key) {
			if ($param == $key->nama_pj) {
			$data['row'] = $this->penindakan_model->get_all_data2019("tbl_kegiatan_penindakan", "nama_pj", "tanggal", $param, 2018)->result_array();
			$data['total'] = $this->penindakan_model->get_total_kegiatan(2018)->row_array();
			$data['subtotal'] = $this->penindakan_model->get_total_subkegiatan(2018)->row_array();
			$this->load->view("penindakan/spuser/cetak",$data);
